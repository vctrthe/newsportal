...
func (coh *contentHandler) GetContentWithQuery(c *fiber.Ctx) error {
	page := 1
	if c.Query("page") != "" {
		...
		if err != nil {
			...
			log.Errorw(code, err)
			errorResp.Meta.Status = false
			errorResp.Meta.Message = "Invalid page parameter"
	...
	if c.Query("limit") != "" {
		...
		if err != nil {
			...
			log.Errorw(code, err)
			errorResp.Meta.Status = false
			errorResp.Meta.Message = "Invalid limit parameter"
	...
	if err != nil {
		code = "[HANDLER] GetContentsWithQuery - 4"
		log.Errorw(code, err)
		errorResp.Meta.Status = false
		errorResp.Meta.Message = err.Error()
...
func (coh *contentHandler) CreateContent(c *fiber.Ctx) error {
	...
	regEntity := entity.ContentEntity{
		Title:       req.Title,
		Excerpt:     req.Excerpt,
	...
	err = coh.contentService.CreateContent(c.Context(), regEntity)
	if err != nil {
...
func (coh *contentHandler) UpdateContent(c *fiber.Ctx) error {
	...
	err = coh.contentService.UpdateContent(c.Context(), regEntity)
	if err != nil {
...
func (coh *contentHandler) UploadImageR2(c *fiber.Ctx) error {
	...
	imageUrl, err := coh.contentService.UploadImageR2(c.Context(), reqEntity)
	if err != nil {
